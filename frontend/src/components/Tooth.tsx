...
export default function Tooth({ number }: ToothProps) {
    ...
    switch (typeMain) {
      ...
      case "Fiber Post":
        return {
          ...
          h: 30,
          node: (
            <g transform="translate(-1 22) scale(0.9) translate(-4 -22.5)">
              <path
      ...
      case "Stainless Crown":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0,55) translate(0,22) scale(1,-1)">
              <path
      ...
      case "Metal Post":
        return {
          ...
          h: 36,
          node: (
            <g transform="translate(3,11) scale(1.2)">
              <path
      ...
      case "Composite":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0,55) translate(0,25) scale(1,-1)">
              <path
      ...
      case "Amalgam":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0,58) translate(0,22) scale(1,-1)">
              <path
                d="M13.2298 19.462C6.53983 19.982 -0.420172 13.132 0.019828 5.61203C0.239828 1.89203 2.45983 -0.237976 5.98983 0.332024C10.9398 1.13202 15.8098 1.17202 20.7598 0.312024C24.2198 -0.287976 26.4298 1.98202 26.6098 5.74202C26.9598 12.852 20.1798 20.082 13.2298 19.462Z"
                fill="#1F201E"
              />
      ...
      case "Inlay":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0 55) translate(0,20) scale(1,-1)">
              <path
      ...
      case "Onlay":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0, 58) translate(0,20) scale(1,-1)">
              <path
      ...
      case "Zirconia":
        return {
          ...
          h: 2,
          node: (
            <g transform="translate(0, 55) treanslate(0, 22) scale(1,-1)">
              <path
  ...
  // Replace the renderOverlay function with this version
  // Replace the renderOverlay function with this version
  const renderOverlay = (proc: Procedure, idx: number) => {
    ...
    // Get quadrant and position info
    const tens = Math.floor(number / 10);

    // Determine tooth position and side
    const isUpperTooth = tens === 1 || tens === 2 || tens === 5 || tens === 6;
    const isLowerTooth = tens === 3 || tens === 4 || tens === 7 || tens === 8;
    const isLeftSide = tens === 1 || tens === 4 || tens === 5 || tens === 8;
    const isRightSide = tens === 2 || tens === 3 || tens === 6 || tens === 7;

    // DETERMINE IF TOOTH IS BIG BASED ON YOUR SPECIFICATION
    const isBigTooth = (toothNum: number) => {
      const tens = Math.floor(toothNum / 10);
      const ones = toothNum % 10;

      if (tens === 1 && ones > 5) return true; // Above 15 in 10 generation
      if (tens === 2 && ones > 5) return true; // Above 25 in 20 generation
      if (tens === 3 && ones > 5) return true; // Above 35 in 30 generation
      if (tens === 4 && ones > 5) return true; // Above 45 in 40 generation

      return false;
    };

    const bigTooth = isBigTooth(number);

    // TOOTH SIZE MAPPING - with big tooth consideration
    const getToothSize = (toothNum: number) => {
      // Adult teeth
      if (toothNum >= 11 && toothNum <= 48) {
        // Apply big tooth scaling factor
        const bigFactor = bigTooth ? 1.4 : 1.0;

        switch (toothNum) {
          // Upper anterior - smallest
          case 11:
          case 21:
            return { width: 65 * bigFactor, height: 65 * bigFactor };
          case 12:
          case 22:
            return { width: 65 * bigFactor, height: 65 * bigFactor };
          // Upper canines
          ...
          case 25:
            return { width: 65 * bigFactor, height: 65 * bigFactor };
          // Upper molars - these are naturally big teeth
          case 16:
          case 26:
            return { width: 65 * bigFactor, height: 45 * bigFactor };
          case 17:
          case 27:
            return { width: 65 * bigFactor, height: 45 * bigFactor };
          case 18:
          ...
          case 33:
          case 43:
            return { width: 60 * bigFactor, height: 65 * bigFactor };
          // Lower premolars
          case 34:
          case 44:
            return { width: 65 * bigFactor, height: 65 * bigFactor };
          ...
          case 45:
            return { width: 65 * bigFactor, height: 65 * bigFactor };
          // Lower molars - these are naturally big teeth
          case 36:
          case 46:
            return { width: 65 * bigFactor, height: 45 * bigFactor };
          case 47:
          ...
          case 38:
          case 48:
            return { width: 62 * bigFactor, height: 45 * bigFactor };
          default:
            return { width: 65 * bigFactor, height: 60 * bigFactor };
        }
      }
      // Child teeth (smaller)
      else if (toothNum >= 51 && toothNum <= 85) {
        const bigFactor = bigTooth ? 1.2 : 1.0; // Smaller scaling for child teeth

        switch (toothNum) {
          case 51:
          case 61:
            return { width: 60 * bigFactor, height: 55 * bigFactor };
          case 52:
          case 62:
            return { width: 60 * bigFactor, height: 55 * bigFactor };
          case 53:
    ...
    const toothSize = getToothSize(number);

    // Base positioning
    let posX = 50;
    let posY = 50;

    // PROCEDURE-SPECIFIC SCALING with big tooth consideration
    let scaleX, scaleY;

    // Base scaling factors that get multiplied by big tooth factor
    const baseScaleX = toothSize.width / def.w;
    const baseScaleY = toothSize.height / def.h;

    switch (typeMain) {
      case "Fiber Post":
        scaleX = baseScaleX * 0.2;
        scaleY = baseScaleY * 1.2;
        posY = isUpperTooth ? 85 : 40;
        posX = isLeftSide ? 58 : 65;
        break;

      case "Stainless Crown":
      case "Prosthetic":
        scaleX = isBigTooth(number) ? baseScaleX * 0.9 : baseScaleX * 0.8;
        scaleY = isBigTooth(number) ? baseScaleY * 0.09 : baseScaleY * 0.08;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -72
            : -55
      ...
      case "RCT":
      case "Re-RCT":
        scaleX = baseScaleX * 0.4;
        scaleY = baseScaleY * 1.45;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? 75
            : 80
      ...
      case "Metal Post":
        ...
        scaleY = baseScaleY * 1.65;
        posY = isUpperTooth ? (isBigTooth(number) ? 35 : 30) : 110;
        posX = isBigTooth(number)
          ? isLeftSide
            ? 80
            : 18
      ...
      case "Pulpotomy":
      case "Pulpectomy":
        scaleX = baseScaleX * 0.6;
        scaleY = baseScaleY * 0.4;
        posY = isUpperTooth ? 35 : 6;
        break;
      ...
      case "Composite":
        scaleX = isBigTooth(number) ? baseScaleX * 0.7 : baseScaleX * 0.65;
        scaleY = isBigTooth(number) ? baseScaleY * 0.08 : baseScaleX * 0.77;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -62
            : -70
      ...
      case "Amalgam":
        scaleX = isBigTooth(number) ? baseScaleX * 0.7 : baseScaleX * 0.65;
        scaleY = isBigTooth(number) ? baseScaleY * 0.08 : baseScaleX * 0.95;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -63
            : -110
      ...
      case "Inlay":
        scaleX = isBigTooth(number) ? baseScaleX * 0.73 : baseScaleX * 0.7;
        scaleY = isBigTooth(number) ? baseScaleY * 0.08 : baseScaleY * 0.06;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -49
            : -5
      ...
      case "Onlay":
        scaleX = isBigTooth(number) ? baseScaleX * 0.73 : baseScaleX * 0.6;
        scaleY = isBigTooth(number) ? baseScaleY * 0.07 : baseScaleY * 0.05;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -25
            : 15
      ...
      case "Implant":
        ...
        posY = isUpperTooth ? 80 : 45;
        break;

      case "Veneer":
        scaleX = isBigTooth(number) ? baseScaleX * 0.4 : baseScaleX * 0.4;
        scaleY = isBigTooth(number) ? baseScaleY * 0.07 : baseScaleY * 0.05;
        posY = isUpperTooth
          ? isBigTooth(number)
            ? -25
    ...
    // Mirror horizontally for right side teeth (except cross procedures)
    if (
      isRightSide &&
      ![
        "Apicoectomy",
    ...
    if (["Endo", "Metal Post"].includes(typeMain)) {
      if (isUpperTooth) {
        finalScaleY = Math.abs(scaleY); // Point down for upper
        if (isLeftSide) finalScaleX = -Math.abs(scaleX);
        else finalScaleX = Math.abs(scaleX);
      } else {
        finalScaleY = -Math.abs(scaleY); // Point up for lower
        if (isLeftSide) finalScaleX = -Math.abs(scaleX);
        else finalScaleX = Math.abs(scaleX);
      }
    } else if (
      ...
      if (isLowerTooth) {
        finalScaleY = -Math.abs(scaleY); // Flip for lower teeth
      }
    } else if (typeMain === "Implant") {
      if (isUpperTooth) {
        finalScaleY = Math.abs(scaleY);
      } else {
        finalScaleY = -Math.abs(scaleY);
      }
    } else if (typeMain === "Zirconia") {
      if (isUpperTooth) finalScaleY = -Math.abs(scaleY);
    } else if (typeMain === "Fiber Post") {
      if (isUpperTooth) {
        finalScaleY = -Math.abs(scaleY);
      } else {
        finalScaleY = Math.abs(scaleY);
    ...
    if (["RCT", "Re-RCT"].includes(typeMain)) {
      if (isUpperTooth) {
        finalScaleY = Math.abs(scaleY); // Point down for upper
        if (isLeftSide) finalScaleX = Math.abs(scaleX);
        else finalScaleX = -Math.abs(scaleX);
    ...
    return (
      <g
        key={`ov-${idx}`}
        {...sharedHandlers}
        transform={`translate(${posX} ${posY}) scale(${finalScaleX} ${finalScaleY}) translate(${txBefore} ${tyBefore})`}
      >
        {def.node}
      </g>
    );
