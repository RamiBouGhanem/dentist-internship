...
`);
export function readStore(){return JSON.parse(db.prepare('SELECT data FROM store WHERE id=1').get().data)}
...
if(!db.prepare('SELECT id FROM store WHERE id=1').get()){
 ...
 s.assignments=[];s.checkins=[];s.messages=[];s.bookings=[];s.transformations=[];
