...
function App() {
  ...
  async function payForProgram(provider) {
    ...
    try {
      ...
      if (session.role !== 'member') { location.href = '/member?return=program&program=' + selected.id; return; }
  ...
  useEffect(() => {
    ...
    if (payId && catalog.length) {
      ...
      if (program) { setSelected(program); setSubmitted(null); setOrderError(''); }
...
createRoot(document.getElementById('root')).render(<React.StrictMode>{window.location.pathname.startsWith('/admin') ? <Admin /> : window.location.pathname.startsWith('/payment/return') ? <PaymentReturn /> : window.location.pathname.startsWith('/member') ? <Member /> : <App />}</React.StrictMode>);
